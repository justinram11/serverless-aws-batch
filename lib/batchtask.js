...
function getJobDefinitionLogicalId(functionName) {
    ...
    return logicalId.length > 64 ? logicalId.substring(0, 64) : logicalId;
}
 ...
 */
async function compileBatchTask(functionName) {
    ...
    const functionArtifactFileName = this.provider.naming.getLambdaScheduleArtifactName(functionName);
    // Now replace the original lambda function with code to invoke this batch task
    generateLambdaScheduleArtifact.bind(this)(functionName, functionArtifactFileName);

    _.merge(this.serverless.service.functions, {
        [functionName]: {
            handler: 'handler.schedule',
            name: functionObject.name,
            memorySize: 128,
            timeout: 6,
            runtime: getRuntime.bind(this)(),
            package: {
                artifact: path.join(this.serverless.config.servicePath, '.serverless', functionArtifactFileName),
            },
            environment: {
                EVENT_LOGGING_ENABLED:
                    functionObject.batch.hasOwnProperty('scheduleLoggingEnabled') && functionObject.batch.scheduleLoggingEnabled,
            ...
            role: {
                'Fn::GetAtt': [this.provider.naming.getLambdaScheduleExecutionRoleLogicalId(), 'Arn'],
            },
        },
    });
...
function getRuntime() {
    const runtime = this.serverless.service.provider.runtime,
        isNodeJS = runtime.includes('nodejs');

    return isNodeJS ? runtime : 'nodejs14.x';
}
...
function updateContainerProperties(newFunction, functionObject, functionName) {
    newFunction.Properties.ContainerProperties = _.merge({}, functionObject.batch.ContainerProperties);
    ...
    if (!newFunction.Properties.ContainerProperties.hasOwnProperty('Command')) {
        ...
        newFunction.Properties.ContainerProperties.Command = [`${this.serverless.service.service}/${functionObject.handler}`, 'Ref::event'];
    }

    if (!newFunction.Properties.ContainerProperties.hasOwnProperty('Image')) {
        newFunction.Properties.ContainerProperties.Image = this.provider.naming.getDockerImageName(functionName);
    }
    ...
    _.forEach(_.keys(environment), (key) => {
        ...
        if (_.isObject(value)) {
            const isCFRef = _.isObject(value) && !_.some(value, (v, k) => k !== 'Ref' && !_.startsWith(k, 'Fn::'));
            if (!isCFRef) {
...
function updateRetryStrategy(newFunction, functionObject) {
    newFunction.Properties.RetryStrategy = _.merge({}, functionObject.batch.RetryStrategy);
...
function updateTimeout(newFunction, functionObject) {
    newFunction.Properties.Timeout = _.merge({}, functionObject.batch.Timeout);
...
function getMemorySize(functionObject) {
    return Number(functionObject.batch.memory) || Number(functionObject.memory) || 2048;
}
...
function getTimeout(functionObject) {
    return (
        Number(functionObject.batch.Timeout ? functionObject.batch.Timeout.AttemptDurationSeconds : undefined) ||
 ...
 */
async function generateLambdaScheduleArtifact(functionName, functionArtifactFileName) {
...
function generateBatchJobExecutionRole(functionName, functionObject) {
    const batchJobExecutionRoleName = `BatchJobRole-${functionName}-${this.provider.getRegion()}-${this.provider.getStage()}-${
    ...
    if (this.serverless.service.provider.hasOwnProperty('iamRoleStatements')) {
        _.merge(batchJobExecutionRole.Properties, {
            Policies: [
                {
 ...
 */
async function compileBatchTasks() {
    const allFunctions = this.serverless.service.getAllFunctions();

    for (const f of allFunctions) {
        await compileBatchTask.bind(this)(f);
    }
}
