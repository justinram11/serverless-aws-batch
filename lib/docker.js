...
function buildDockerImage() {
    ...
    if (this.serverless.service.provider.runtime === 'nodejs8.10') { 
        dockerFileContents.concat('ENTRYPOINT ["/var/lang/bin/node", "--expose-gc", "--max-semi-space-size=150", "--max-old-space-size=30000", "/var/runtime/node_modules/awslambda/index.js"]\n');
    } else if (this.serverless.service.provider.runtime === 'nodejs10.x') {
        dockerFileContents.concat('ENV NODE_OPTIONS="--max-old-space-size=30000"\n');
    } else {
        dockerFileContents.concat('ENV NODE_OPTIONS="--max-old-space-size=30000"\n');
    }
