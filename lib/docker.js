...
function buildDockerImage() {
    ...
    `
    RUN pip install -U awscli -t /tmp --no-cache-dir --ignore-installed six
    ...
    COPY --from=builder /var/lang/bin /var/task/${this.serverless.service.service}/
    RUN rm -rf /tmp/\*
    ENV ASSUME_ROLE_ARN=${this.provider.naming.getLambdaScheduleExecutionRoleLogicalId()}
    ENV PATH=\${PATH}:/var/task/${this.serverless.service.service}/
    `;
    ...
    ENTRYPOINT /bin/bash -c '/var/task/${this.serverless.service.service}/aws_environment.sh && /var/lang/bin/python3.6 /var/runtime/awslambda/bootstrap.py'
    `
    )};
