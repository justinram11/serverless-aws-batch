...
function getLambdaScheduleArtifactName(functionName) {
  return `${functionName}-schedule.zip`;
}
 ...
 *  - securityGroups
 */
async function validateAWSBatchServerlessConfig() {
...
function generateBatchServiceRole() {
    const batchServiceRoleName = `BatchServiceRole-${this.provider.getRegion()}-${this.provider.getStage()}-${
...
function generateBatchInstanceRole() {
    const batchInstanceRoleName = `BatchInstanceRole-${this.provider.getRegion()}-${this.provider.getStage()}-${
        this.provider.serverless.service.service
    }`.substring(0, 64);
...
function generateBatchSpotFleetRole() {
    const batchSpotRoleName = `BatchSpotFleetRole-${this.provider.getRegion()}-${this.provider.getStage()}-${
        this.provider.serverless.service.service
    }`.substring(0, 64);
...
function generateLambdaScheduleExecutionRole() {
    const lambdaScheduleExecutionRoleName = `BatchScheduleRole-${this.provider.getRegion()}-${this.provider.getStage()}-${
        this.provider.serverless.service.service
    }`.substring(0, 64);
...
function generateBatchComputeEnvironment() {
    ...
    const batchComputeResourceObject = _.merge({}, JSON.parse(batchComputeResourceTemplate), this.serverless.service.provider.batch);

    // If we are a SPOT type, default the BigPercentage to 100% (always pay lowest market price)
    if (
        batchComputeResourceObject.hasOwnProperty('Type') &&
    ...
    const computeEnvironmentName = `${this.provider.serverless.service.service}-${this.provider.getStage()}-ComputeEnvironment-${Math.floor(
 ...
 * template
 */
async function generateAWSBatchTemplate() {
